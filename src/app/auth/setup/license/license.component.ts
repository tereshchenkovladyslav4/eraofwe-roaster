...
export class LicenseComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        var language = 'en';
        this._userService.getUserLanguageStrings(language).subscribe((resultLanguage) => {
    ...
    savecertificate(rowcount, event) {
        for (let j = 0; j < this.licenseArray.length; j++) {
            if (this.licenseArray[j].name == '' && this.licenseArray[j].year == '' && this.licenseArray[j].type == '') {
                $('.myAlert-top').show();
