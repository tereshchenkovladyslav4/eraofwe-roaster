...
export class OrderChatComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.currentDispute_id = this.route.snapshot.queryParams['ticketId']
    ...
    getOrderDetails() {
        this.roasterService.getViewOrderDetails(this.roaster_id, this.orderID, this.orderType).subscribe(
            (res) => {
                console.log(res);
    ...
    getOrderDisputes() {
        this.orderDisputes = [];
        this.roasterService.getOrderDisputeList(this.roaster_id, this.orderID, this.orderType).subscribe(
            (res) => {
                console.log(res);
                if (res['success'] && res['result']) {
    ...
    markResolved() {
        this.roasterService.markTicketasResolved(this.roaster_id, this.currentDispute_id).subscribe(
            (res) => {
                console.log(res);
                if (res['success']) {
    ...
    escalateTicket() {
        this.roasterService.escalteTicket(this.roaster_id, this.currentDispute_id).subscribe(
            (res) => {
                console.log(res);
                if (res['success']) {
    ...
    ngAfterViewInit() {
        ...
        $('body').on('click', '.share-ur-knowledge-main__submit .submit', function () {
            var ChatText = $(this)
                .parents('.share-ur-knowledge-main__submit')
                .find('.q-a-forum-input')
                .find('.chat-inputs__text')
                .val();
            ...
            if (ChatText !== '' && ChatImg !== '') {
                var message =
                    '<div class="main-message message-to"><div class="main-message__profile clearfix"><div class="float-left img"><img src="assets/images/profile.png"></div><div class="profile-name"><div class="float-left"><h4 class="name">Finca La Pampa</h4><span class="status">Facilitator</span></div></div></div><div class="message-info"><div class="message-info__body position-relative"> <div class="message-info__img">' +
                    img +
                    '</div><p class="message-text">' +
                    ChatText +
                    '</p></div><span class="live-time">' +
                    strTime +
                    '</span></div></div>';

                var mesbdy = $(this)
                    .parents('.share-ur-knowledge-main__submit')
                    .find('.conversatio-body__messages')
                    .append(message);
                var clrInput = $(this)
                    .parents('.share-ur-knowledge-main__submit')
                    .find('.q-a-forum-input')
                ...
                var message =
                    '<div class="main-message message-to"><div class="main-message__profile clearfix"><div class="float-left img"><img src="assets/images/profile.png"></div><div class="profile-name"><div class="float-left"><h4 class="name">Finca La Pampa</h4><span class="status">Facilitator</span></div></div></div><div class="message-info"><div class="message-info__body position-relative"><p class="message-text px-2">' +
                    ChatText +
                    '</p></div><span class="live-time">' +
                    strTime +
                    '</span></div></div>';
