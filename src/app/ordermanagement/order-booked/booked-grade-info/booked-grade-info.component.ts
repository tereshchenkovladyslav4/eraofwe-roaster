...
export class BookedGradeInfoComponent implements OnInit {
    ...
    getHarvestDetails() {
        this.harvestDetails = [];
        this.roasterService.getHarvestDetails(this.bookedService.harvestId).subscribe(
            (res) => {
    ...
    viewReport(item?) {
        this.roasterService.getCuppingReportDetails(this.bookedService.harvestId).subscribe(
            (res) => {
