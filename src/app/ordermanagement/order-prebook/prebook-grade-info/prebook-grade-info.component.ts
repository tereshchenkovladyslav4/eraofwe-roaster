...
export class PrebookGradeInfoComponent implements OnInit {
    ...
    getHarvestDetails() {
        this.harvestDetails = [];
        this.roasterService.getHarvestDetails(this.prebookService.harvestId).subscribe(
            (res) => {
    ...
    viewReport(item?) {
        this.roasterService.getCuppingReportDetails(this.prebookService.harvestId).subscribe(
            (res) => {
