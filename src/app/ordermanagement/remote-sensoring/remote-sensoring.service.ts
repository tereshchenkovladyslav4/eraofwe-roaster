...
export class RemoteSensoringService {
  ...
  viewPrebookOrderDetails() {
    this.roasterService
      .getViewOrderDetails(this.roasterId, this.orderPreId)
      .subscribe((res) => {
        if (res["success"] == true) {
          ...
          this.estate_thumbnail_url =
            res["result"]["estate_profile_image_thumbnail_url"];
          ...
          this.roaster_thumbnail_url =
            res["result"]["roaster_profile_image_thumbnail_url"];
  ...
  viewpreEstateDetails() {
    this.userService
      .getAvailableEstateList(this.roasterId, this.estate_id)
      .subscribe((res) => {
        if (res["success"] == true) {
          this.company_image_thumbnail_url =
            res["result"]["company_image_thumbnail_url"];
  ...
  viewpreLotDetails() {
    this.userService
      .getRoasterLotDetails(this.roasterId, this.estate_id, this.lot_id)
      .subscribe((res) => {
        if (res["success"] == true) {
  ...
  ViewRecentActivity() {
    this.userService
      .getRecentActivity(this.roasterId, this.orderPreId)
      .subscribe((res) => {
        if (res["success"] == true) {
          console.log(res);
