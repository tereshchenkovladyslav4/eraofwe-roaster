...
export class MicroOrderBookedComponent implements OnInit {
  ...
  save() {
    ...
    this.userService
      .updateShipmentDetails(this.roasterId, this.bookId, data)
      .subscribe((data) => {
  ...
  verifyPayment() {
    this.userService
      .updatePaymentVerify(this.roasterId, this.bookId)
      .subscribe((data) => {
        if (data["success"] == true) {
  ...
  verifyPaymentAfterDelivery() {
    this.userService
      .updatePaymentAfterDelivery(this.roasterId, this.bookId)
      .subscribe((data) => {
        if (data["success"] == true) {
          ...
          this.toastrService.error(
            "Error while verifying the payment after delivery"
          );
        }
      });
  }
  ...
  viewMrOrderDetails() {
    this.roasterService
      .getMrGcOrderDetails(this.roasterId, this.bookId)
      .subscribe((res) => {
        if (res["success"] == true) {
  ...
  timelineData() {
    ...
    if (
      this.bookDetailService.paymentVerification == true &&
      this.bookDetailService.status == "CONFIRMED"
    ) {
      setTimeout(() => {
        this.sampleValueToShow = "Payment";
        this.paySample();
