...
export class NewDirectMessageComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  updateUserStatus() {
    ...
    if (this.userStatusTimerRef) {
      ...
      this.userStatusTimerRef = setTimeout(this.updateUserStatus, this.UPDATE_USER_STATUS_INTERVAL) as unknown as number;
    }
  }
  updateUnRead() {
    // Fetch unread
  ...
  viewPortSizeChanged = () => {
    ...
    const liveChatHead = (this.elRef?.nativeElement?.querySelector('[data-element="live-chat-head"]')) as HTMLElement || null;
    const chatMessageForm = (this.elRef?.nativeElement?.querySelector('[data-element="message-form"]')) as HTMLElement || null;
    const liveChatBody = (this.elRef?.nativeElement?.querySelector('[data-element="live-chat-message-body"]')) as HTMLElement || null;

    const totalHeight = (chatAccountHead?.offsetHeight || 0) + (liveChatHead?.offsetHeight || 0) + (chatMessageForm?.offsetHeight || 0);
    if (liveChatBody) {
