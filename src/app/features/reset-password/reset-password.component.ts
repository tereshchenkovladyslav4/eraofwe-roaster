...
export class ResetPasswordComponent implements OnInit {
    ...
    changePassword() {
        ...
        if (this.currentPassword == '' || this.currentPassword == null || this.currentPassword == undefined) {
            ...
            document.getElementById('currentPassword').style.border = '1px solid #D50000 ';
            setTimeout(() => {
                this.currentPasswordError = '';
            }, 3000);
                ...
                if (response['success'] == true) {
                    ...
                    if (response['messages']['current_password'] !== undefined) {
                        ...
                        this.toastrService.error(
                            'Error: Current password is ' + response['messages'].current_password[0] + '!',
                        );
                    }
                    if (response['messages']['new_password'] !== undefined) {
                        // console.log("Error: New password is "+response['messages'].new_password[0] +"!");
                        this.toastrService.error(
                            'Error: New password is ' + response['messages'].new_password[0] + '!',
                        );
                    }
                    if (response['messages']['confirm_password'] !== undefined) {
                        ...
                        this.toastrService.error(
                            'Error: Confirm password is ' + response['messages'].confirm_password[0] + '!',
                        );
                    } else {
