      ...
      .subscribe((data) => {
        if (data["success"] == true) {
          ...
          this.cuppingScoreDetailsArray.forEach((ele) => {
            ...
            scoreObj["series"] = [];
            let finalScoreObj = this.formObject(
              "Final score",
              ele["final_score"]
            );
            ...
            let afterStateObj = this.formObject(
              "After taste",
              ele["aftertaste_score"]
            );
            let actidityObj = this.formObject(
              "Acidity",
              ele["aftertaste_score"]
            );
            let bodyObj = this.formObject("Body", ele["body_score"]);
            let balanceObj = this.formObject("Balance", ele["balance_score"]);
            let UniformityObj = this.formObject(
              "Uniformity",
              ele["uniformity_score"]
            );
            let cleancupObj = this.formObject(
              "Clean cup",
              ele["cleancup_score"]
            );
            let sweetnessObj = this.formObject(
              "Sweetness",
              ele["sweetness_score"]
            );
          ...
          this.finalscores = this.avgScoreArray.reduce(
            (acc, cur) => acc + cur,
            0
          );
          ...
          this.toastrService.error(
            "Error while getting the Cupping score details"
          );
        }
      });
  }
