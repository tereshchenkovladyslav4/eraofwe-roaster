...
export class GenerateGreenCoffeeComponent implements OnInit {
  ...
  // }
  evaluatorsList(){
    this.userService.getEvaluatorsList(this.ro_id,this.cupping_report_id).subscribe(
      response => {
  ...
  getUserRole(val : any){
    this.roasterService.getUserBasedRoles(this.ro_id,val).subscribe(
      data => {
  ...
  getUsersList(e :any){
    ...
    this.roasterService.getRoasterUsers(this.ro_id).subscribe(
      data => {
        if(data['success']==true){
  ...
  addEvaluators(){
    ...
    if(this.selectedValue != ""){
      ...
      this.userService.addEvaluators(this.cupping_report_id,data).subscribe(
        res => {
          if(res['success'] == true){
  ...
  goNext(){
    if(this.cupping == "" || this.sample_size == undefined){
      ...
      this.userService.updateCuppingType(this.cupping_report_id,data).subscribe(
        res => {
          res.success ? this.toastrService.success("Cupping type updated") : this.toastrService.error("Error while Updating")
        }
      )
