...
export class GenerateMySampleComponent implements OnInit {
    ...
    getCuppingScoreDetails() {
        this.userService
            .getCuppingScore(this.roaster_id, this.cupping_report_id, this.singleCuppingDetails.evaluator_id)
            .subscribe((res) => {
                if (res['success'] == true && res['result'].length > 0) {
                    ...
                    this.getFlavourArray = res['result'][0].descriptors ? res['result'][0].descriptors.split(',') : [];
                    this.flavourProfileList();
                } else {
                    this.flavourProfileList();
                }
    ...
    evaluatorsList() {
        this.userService.getEvaluatorsList(this.roaster_id, this.cupping_report_id).subscribe((response) => {
    ...
    goNext() {
        if (this.cupping_report_id == undefined) {
            if (this.final_score == undefined || this.overall_score == undefined || this.comments == '') {
                ...
                this.userService.addCuppingScore(this.roaster_id, this.cupping_report_id, data).subscribe((data) => {
                    if (data['success'] == true) {
