...
@Component({
    selector: 'app-micro-roaster-agreements',
    templateUrl: './micro-roaster-agreements.component.html',
    styleUrls: ['./micro-roaster-agreements.component.css'],
})
export class MicroRoasterAgreementsComponent implements OnInit {
    estateterm: any;
    ...
    openFileModal(template: TemplateRef<any>, itemId: any) {
        this.modelMrref = this.modalService.show(template);
        this.roasterService.getAgreementValue(this.roasterId, this.customer_type, itemId).subscribe((data) => {
    ...
    openDeleteModal(deleteTemplate: TemplateRef<any>, deleteId: any) {
        this.modelMrref = this.modalService.show(deleteTemplate);
    ...
    ngOnInit(): void {
        ...
        // 	})
        // }
        this.customer_type = 'micro-roasters';
    ...
    getHorecaList() {
        this.roasterService.getMicroRoastersList(this.roasterId).subscribe((res) => {
            if (res['success'] == true) {
    ...
    getAgreements() {
        this.roasterService.getAgreements(this.roasterId, this.customer_type).subscribe((data) => {
            if (data['success'] == true) {
    ...
    uploadAgreement() {
        ...
        if (this.customer_id == '' || this.customer_id == null || this.customer_id == undefined) {
            this.customerIdError = 'Please Select Customer Id';
            document.getElementById('customer').style.border = '1px solid #D50000 ';
            setTimeout(() => {
            ...
            if (fileList.length > 0) {
                ...
                this.roasterService.uploadFiles(formData).subscribe((result) => {
                    if (result['success'] == true) {
                        ...
                        this.roasterService
                            .uploadAgreements(this.roasterId, this.customer_type, data)
                            .subscribe((res) => {
                                if (res['success'] == true) {
        ...
        if (fileList.length > 0) {
            ...
            this.roasterService.uploadFiles(formData).subscribe((result) => {
                if (result['success'] == true) {
                    ...
                    this.roasterService
                        .updateAgreements(this.roasterId, this.customer_type, this.item_id, data)
                        .subscribe((res) => {
                            if (res['success'] == true) {
    ...
    deleteAgreement(item: any) {
        console.log(item);
        this.roasterService.deleteAgreement(this.roasterId, this.customer_type, item).subscribe((res) => {
            if (res['success'] == true) {
