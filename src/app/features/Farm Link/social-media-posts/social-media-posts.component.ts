...
export class SocialMediaPostsComponent implements OnInit {
    ...
    getSocialImages() {
        this.userService.getSocialMediaPosts(this.roaster_id, 'IMAGE').subscribe((response) => {
    ...
    getSocialVideos() {
        this.userService.getSocialMediaPosts(this.roaster_id, 'VIDEO').subscribe((response) => {
            if (response['success'] == true) {
