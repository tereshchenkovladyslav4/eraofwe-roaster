...
export class AvailableConfirmOrderComponent implements OnInit {
  ...
  placeOrderMob() {
    if (
      this.quantity1 == "" ||
      this.quantity1 == null ||
      this.quantity1 == undefined
    ) {
      this.confirmOrderError = "Please enter quantity";
      document.getElementById("quantity_id").style.border = "1px solid #D50000";
      setTimeout(() => {
        this.confirmOrderError = "";
        document.getElementById("quantity_id").style.border =
          "1px solid #d6d6d6 ";
      }, 3000);
    } else if (
      this.service == "Import & Delivery service" &&
  ...
  saveAddress() {
    if (
      ...
      document.getElementById("countryList").style.border =
        "1px solid #D50000 ";
      setTimeout(() => {
        this.countryError = "";
        document.getElementById("countryList").style.border =
      ...
      if (this.addressId) {
        ...
        this.userService
          .editAddress(this.roaster_id, this.addressId, addressData)
          .subscribe((response) => {
        ...
        this.userService
          .addAddresses(this.roaster_id, data)
          .subscribe((response) => {
            if (response["success"] == true) {
  ...
  getshipInfo() {
    this.userService
      .getShippingInfo(this.roaster_id, this.sourcing.estate_id)
      .subscribe((response) => {
  ...
  orderSampleDone() {
    ...
    this.userService
      .addRequestSample(this.roaster_id, this.sourcing.harvestData, doneData)
      .subscribe((data) => {
        if (data["success"] == true) {
          this.toastrService.success("Order has been placed Successfully");
          this.router.navigate(["/features/order-placed"]);
