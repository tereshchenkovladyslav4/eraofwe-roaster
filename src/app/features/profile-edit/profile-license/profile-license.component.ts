...
export class ProfileLicenseComponent implements OnInit {
    ...
    getCertificates() {
        this._userService.getCertificates(this.roasterId, this.userId).subscribe((data) => {
    ...
    getCertificateTypes() {
        this._userService.getCertificateTypes().subscribe((res) => {
            if (res['success'] == true) {
    ...
    savecertificate(rowcount, event) {
        for (let j = 0; j < this.licenseArray.length; j++) {
            if (this.licenseArray[j].name == '' && this.licenseArray[j].year == '') {
                ...
                document.getElementById('certification_type').style.border = '1px solid #d50000';
                setTimeout(() => {
                    this.certificationNameError = '';
                    this.certificationYearError = '';
                    this.certificationTypeError = '';
                ...
                if (rowcount == this.licenseArray[j].id) {
                    ...
                    if (this.licenseArray[j].uploadFileFlag == 'upload') {
                        ...
                        if (fileList.length > 0) {
                            ...
                            this._userService.uploadCertificate(formData).subscribe((uploadResult) => {
    ...
    deleteCertificate(certificateId: any) {
        if (confirm('Please confirm! you want to delete?') == true) {
            this._userService.deleteCertificate(this.roasterId, this.userId, certificateId).subscribe((response) => {
